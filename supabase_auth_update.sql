-- ═══════════════════════════════════════════════════════════════════════════
--  PAES Prep · Schema v2 — Auth email + tabla colegios
--  Correr en: Supabase → SQL Editor
--
--  ANTES de correr este script:
--  1. Dashboard → Authentication → Providers → Deshabilitar "Google"
--  2. Dashboard → Authentication → Providers → Habilitar "Email"
--  3. Dashboard → Authentication → Settings → Enable "Confirm email" = ON
--  4. Dashboard → Authentication → URL Configuration → Site URL:
--       http://localhost:5173  (dev)  ó tu dominio de producción
-- ═══════════════════════════════════════════════════════════════════════════

-- ── 1. Nuevas columnas en usuarios ──────────────────────────────────────────
ALTER TABLE usuarios
  ADD COLUMN IF NOT EXISTS anio_nacimiento INTEGER,
  ADD COLUMN IF NOT EXISTS situacion       TEXT DEFAULT 'estudiante',
  ADD COLUMN IF NOT EXISTS region          TEXT,
  ADD COLUMN IF NOT EXISTS colegio_id      INTEGER;

-- ── 2. Tabla de colegios (enseñanza media · fuente: MINEDUC) ────────────────
--  Para importar la lista completa visita: https://datosabiertos.mineduc.cl/
--  Filtrar por: nivel_ensenanza = 'Enseñanza Media'
CREATE TABLE IF NOT EXISTS colegios (
  ...
  comuna  TEXT,
  tipo    TEXT                  -- 'Municipal' | 'Part. Subvencionado' | 'Part. Pagado' | 'Corp. Municipal'
);

CREATE INDEX IF NOT EXISTS idx_colegios_region ON colegios(region);

-- ── 3. Seed — representativo por región (datos iniciales MVP) ────────────────
